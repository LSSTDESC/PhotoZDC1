...
class TestCalcMag(unittest.TestCase):
    ...
    def test_throw_no_filter_error(self):
        ...
        p = phot.CalcMag(self.testsed, self.testfilters, self.cosmomodel)
    ...
    def test_reasonable_magnitude_range(self):
        ...
        for sed in self.sedlist:
            ...
            p = phot.CalcMag(sed, self.testfilters, self.cosmomodel)
...
class TestFractionalErrorModel(unittest.TestCase):
        
        
    def test_error_never_negative(self):
        """Test magnitude error is never negative"""
    
        ep = pem.FractionalErrorModel()
        ...
        for i in xrange(nMag):
            ...
            obsmag, errorMag = ep.getObs(mag)
            
            self.assertGreater(errorMag, 0.)
        
        
    def test_raise_error_when_mag_inf(self):
        """Test error calculation throws an error if the input mag is inf"""
    
        ep = pem.FractionalErrorModel()
        
        mag = float('inf')
        self.assertRaises(ValueError, lambda: ep.getObs(mag) )
        
        
class TestLsstErrorModel(unittest.TestCase):


    def setUp(self):
        """Load in the list of filters"""
        self.filterlist = FILTERLIST
    ...
    def test_error_never_negative(self):
        ...
        for i in xrange(nMag):
            ...
            obsmag, errorMag = ep.getObs(mag, self.filterlist[0])
    ...
    def test_raise_error_when_mag_inf(self):
        ...
        mag = float('inf')
        self.assertRaises(ValueError, lambda: ep.getObs(mag, self.filterlist[0]) )
